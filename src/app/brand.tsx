...
export function Logo({ size = 40 }: { size?: number }): React.JSX.Element {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden>
      ...
      <circle cx={c} cy={c} r={size * 0.16} fill="#0a0f14" />
      <circle cx={c} cy={c} r={size * 0.16} fill="none" stroke="#eaf0f6" strokeWidth={size * 0.07} />
    </svg>
  );
}
...
export function Wordmark({ size = 28 }: { size?: number }): React.JSX.Element {
  return (
    <div className="flex items-center gap-2 select-none">
      <Logo size={size * 1.3} />
      <span className="font-display font-semibold tracking-tight text-[var(--mf-text,#eaf0f6)]" style={{ fontSize: size }}>
        MetroForge
      </span>
    </div>
  );
